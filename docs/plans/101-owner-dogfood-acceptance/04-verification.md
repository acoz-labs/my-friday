...
8. Merge, nominate a fresh candidate, rerun the full native Apple-silicon
   workshop harness, obtain the product owner's exact-candidate receipt, approve, and
...
2. the product owner uses the same exact artifact, completes the bounded hands-on owner
...
1. Land the new authority alongside historical verifiers, update current-path
   ...
   acceptance.
2. Link the implementation merge into both #101 provenance and issue #51's
...
5. Release exactly those bytes through the existing artifact workflow, with
   #51 and #101 represented in the release lifecycle as required by the final
...
7. Unblock #92. Its own acceptance later captures actual assistant migration
...
- Planning PR merged and issue #101 Ready under the repository verifier.
- `PRODUCT_OWNER_ACTORS` configured and injected in both workflows.
- Implementation PR merged, linked to #101 and added to issue #51's complete
...
- New workshop evidence and the product owner's owner receipt pass independently.
