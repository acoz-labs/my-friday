...
contract text, and remove `docs/plans/97-owner-dogfood-acceptance/` before the
implementation PR leaves draft.

## Pull Request And Review Contract

- Use one task-scoped `feature/97-owner-dogfood-acceptance` implementation
  branch from the exact approved `origin/main`.
- Associate the PR with #97 and #51 using explicit top-level reference lines;
