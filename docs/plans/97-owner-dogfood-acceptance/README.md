...
is not presented as complete and is not a prerequisite for Anthony's internal
Alfred migration.
...
- `PRODUCT_OWNER_ACTORS` must exist before acceptance and release preflight and
  name Anthony's external GitHub actor. The plan records no private identity in
...
- **Three roles remain separated, but only one is the user.** The workflow
  ...
  maintainer, contributor, service, or Alfred accounts are never labelled as
  independent users.
...
- **Migration evidence is promised now and collected later.** The owner receipt
  binds issue #92 and states that real Alfred migration evidence is due before
...
no blocking finding remains. Anthony has already approved the product pivot;
