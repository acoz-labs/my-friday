...
`assistant/profile.json`; memory starts with empty categories and no records.
...
The transaction writes an owner-only journal, creates sibling stages,
...
automatic completion cannot be proven. Use `my-friday recover <journal>`;
recovery refuses speculative mutation.
